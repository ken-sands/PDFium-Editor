...
//As above but with a tolerance value, this returns only the lines of text within the rectangle where the left hand edge is within the stated tolerance of the furthest left
//line, this is good for extracting text without picking up unwanted floating parts, ie an address while avoiding a reference or date that is to the right. 
DLLEXPORT int STDCALL FPDFText_GetBoundedText2(FPDF_TEXTPAGE text_page,double left, double top, 
											  double right, double bottom,unsigned short* buffer,int buflen,double tolerance)
{
	if (!text_page) return 0;
	IPDF_TextPage* textpage=(IPDF_TextPage*)text_page;
	CFX_FloatRect rect((FX_FLOAT)left,(FX_FLOAT)bottom,(FX_FLOAT)right,(FX_FLOAT)top);
	CFX_WideString str=textpage->GetParaTextByRect(rect,tolerance);
