...
DLLEXPORT int STDCALL FPDFPageObj_GetObjectType(FPDF_PAGEOBJECT pageObject)
{
	if(!pageObject) return FALSE;
	CPDF_PageObject* pPageObj = (CPDF_PageObject*)pageObject;
	return pPageObj->m_Type;
}

DLLEXPORT void STDCALL FPDFPageObj_GetBBox(FPDF_PAGEOBJECT pageObject,float* left,float* bottom,float* right,float* top)
{
	if(!pageObject) return;// false;
	
	CPDF_PageObject* pPageObj = (CPDF_PageObject*)pageObject;
	//CPDF_ImageObject* pPageObj = (CPDF_ImageObject*)pageObject;
	//CFX_Matrix mtx = pPageObj->m_Matrix;
	//pPageObj->CalcBoundingBox();
	//FX_RECT objRect = pPageObj->GetBBox(&mtx);

	//std::cout<<objRect.left; 
	//std::cout<<pPageObj->m_Left; 
	//*left = (float)objRect.left; 
	//*bottom = (float)objRect.bottom;
	//*right = (float)objRect.right;
	//*top = (float)objRect.top;

	*left = pPageObj->m_Left;// objRect.left; 
	*bottom = pPageObj->m_Bottom;//objRect.bottom;
	*right = pPageObj->m_Right;//objRect.right;
	*top = pPageObj->m_Top;//objRect.top;
	return; //(int)pPageObj->m_Left;
}
...
DLLEXPORT void STDCALL FPDFPageObj_TransformRect(FPDF_PAGE page,double left,double top,double right,double bottom,
							double a, double b, double c, double d, double e, double f)
{
	CPDF_Page* pPage = (CPDF_Page*)page;
	FX_RECT TRect = FX_RECT(left,top,right,bottom);
	FX_DWORD objCount = pPage->CountObjects();
	//wprintf(L"objects found: %i",objCount);
	CFX_AffineMatrix matrix((FX_FLOAT)a,(FX_FLOAT)b,(FX_FLOAT)c,(FX_FLOAT)d,(FX_FLOAT)e,(FX_FLOAT)f);
	for(FX_DWORD i = 0;i < objCount;i++){
		CPDF_PageObject* pPageObj = (CPDF_PageObject*)pPage->GetObjectByIndex(i);
		//CPDF_TextObject* pPageObj = (CPDF_TextObject*)pPage->GetObjectByIndex(i);
		FX_RECT bbox = FX_RECT(pPageObj->m_Left,pPageObj->m_Top,pPageObj->m_Right,pPageObj->m_Bottom);
		//if(TRect.Contains(bbox))
		//{
			pPageObj->Transform(matrix);
