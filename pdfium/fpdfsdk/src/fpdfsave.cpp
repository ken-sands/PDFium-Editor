...
int DefWriteBlock(FPDF_FILEWRITE* pThis, const void* pData, unsigned long size)
{
	//FILE* pdfstream = fopen(pThis->pdfFilename, "wb");
	return fwrite(pData, size, 1, pThis->savefile);
	//fclose
}

DLLEXPORT FPDF_BOOL STDCALL FPDF_SaveToFile(	FPDF_DOCUMENT document,char* filename,
												FPDF_DWORD flags )
{
	bool retval;
	FPDF_FILEWRITE pFileWrite;
	pFileWrite.version = 1;
	pFileWrite.pdfFilename = filename;
	pFileWrite.savefile = fopen(filename, "wb");
	pFileWrite.WriteBlock = &DefWriteBlock;
	retval = _FPDF_Doc_Save(document, &pFileWrite, flags, TRUE , 14);
	fclose(pFileWrite.savefile);
	return retval;
}
