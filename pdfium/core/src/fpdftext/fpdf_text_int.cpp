...
bool higheronpage(const TextLine &a, const TextLine &b)
{
	if (a.baseine == b.baseine)
	{
		return a.leftEdge > b.leftEdge;
	}
	return a.baseine > b.baseine;
}

//This one is a bit slower that the one below as it takes all characters in the rectangle, dedupes, orders by coordinates, then reconstructs.
//it also throws out all lines where the left side does not start within the tolerance of the leftmost line.
CFX_WideString CPDF_TextPage::GetParaTextByRect(CFX_FloatRect rect,double tolerance) const
{
    ...
    int nCount = m_charList.GetSize();
    int pos = 0;
	//start the leftmost as the right side of the box
	FX_FLOAT leftmost = rect.right;
    FX_FLOAT posy = 0;
	FX_FLOAT posx = 0;
	...
	int linenum = 0;
	tlines.push_back(TextLine());
	tlines.at(linenum).strText = L"";
    while (pos < nCount) {
        PAGECHAR_INFO charinfo = *(PAGECHAR_INFO*)m_charList.GetAt(pos++);
        if (IsRectIntersect(rect, charinfo.m_CharBox)) {
			//wprintf(L"char: %c x: %f newx: %f max: %f \n",charinfo.m_Unicode,posx,charinfo.m_OriginX,charinfo.m_pTextObj->GetSpaceCharWidth() * 4);
			bool gapped = posx > 0 && FXSYS_fabs(posx - charinfo.m_OriginX) > charinfo.m_pTextObj->GetSpaceCharWidth() * 4; //at least 4 spaces apart then count it as a new string
            if ((FXSYS_fabs(posy - charinfo.m_OriginY) > 0 && !IsContainPreChar && ISAddLineFeed) || gapped) {
                posy = charinfo.m_OriginY;
				posx = charinfo.m_OriginX;
            ...
            if (charinfo.m_Unicode) {
				if(tlines.at(linenum).strText.IsEmpty())
				{
					tlines.at(linenum).leftEdge = charinfo.m_OriginX;
					tlines.at(linenum).baseine = charinfo.m_OriginY;
					if(charinfo.m_OriginX < leftmost)
					{
						leftmost = charinfo.m_OriginX;
					}
				}
				posx = charinfo.m_OriginX;
				tlines.at(linenum).strText += charinfo.m_Unicode;
	...
	// now add the tolderance to the leftmost number and delete all that dont make the grade
	leftmost += tolerance;
//	wprintf(L"Leftedge plus tolerance is %f \n",leftmost);
	for(int i = tlines.size()-1;i>=0;i--)
	{
		if (tlines[i].leftEdge > leftmost || tlines[i].strText.IsEmpty())
		{
			//wprintf(tlines[i].strText+L" %f \n",tlines[i].leftEdge);
			//wprintf(L"removing element %i \n",i);
...
//		
		 ...
		 {
//			std::vector<TextLine>::iterator iter = tlines.begin();
//			iter += i+1;
//			while(iter != tlines.end())
//			{
			 //strText += L"comparing: "+tlines.at(i).strText+L" with "+tlines.at(j).strText + L"\n");
			 //std::cout<<"comparing: "<<tlines.at(i).strText<<" with "<<tlines.at(j).strText;
				if(tlines.at(i).baseine == tlines.at(j).baseine && tlines.at(i).leftEdge == tlines.at(j).leftEdge && tlines.at(i).strText == tlines.at(j).strText)
				{
...
//
//
//			//if(tlines.at(i).baseine == tlines.at(j).baseine && tlines.at(i).leftEdge == tlines.at(j).leftEdge)
//			//{
//			//	if(tlines.at(i).strText == tlines.at(j).strText)
//			//	{
//			//		.;
//			//	}
//			//}
...
//
	...
	//sort the vector by vertical position
	std::sort(tlines.begin(),tlines.end(),higheronpage);

	strText = L"";
	for(int i = 0;i<tlines.size();i++)
	{
		tlines[i].strText.TrimLeft();
		tlines[i].strText.TrimRight();
		strText += tlines[i].strText + L"\r\n";
	}
	//}
	//catch(int e)
	//{
	//	std::cout<<"exception occured near: "<<e<<'\n';
	//}
    return strText;
}
...
void CPDF_TextPage::ProcessTextObject(PDFTEXT_Obj Obj)
{
    ...
    // this code is removing some correct charaters               // bDel = TRUE;
                    break;
                }
            }
            if(!bDel) {
